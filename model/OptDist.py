...
import pandas as pd
from config import all_columns,vocabulary_size
from utils import _normalized_rmse,cumulative_true,gini_from_gain,corr_zero_inflate
...
class OptDist(object):
    ...
    def _init_graph_OptDist(self):
        ...
        with self.graph.as_default():
            ...
            # Variables init.
            self.cvr_label = tf.compat.v1.placeholder(
                tf.float32, shape=[None, 1], name='convert')
            self.labels_fee = tf.compat.v1.placeholder(
                tf.float32, shape=[None, 1], name='labels_fee')
            self.nb_click = tf.compat.v1.placeholder(
            ...
            for column in all_columns:
                self.inputs_placeholder.append(tf.compat.v1.placeholder(
            ...
            feature_embedding = []
            num_distribution = 4
            with tf.compat.v1.variable_scope('fee_part'):
                ...
                H = tf.compat.v1.layers.dense(feature_embedding, 1024, activation=tf.nn.relu, name="share_layer")
                for i in range(num_distribution):
                    mu_out = tf.compat.v1.keras.layers.Dropout(
                        1 - self.keep_prob[0])(H)
                    mu_cvr_vec = tf.compat.v1.layers.dense(mu_out, 512 // 2, activation=tf.nn.relu,
                                                 name="vector_layer_first" + str(i))
                    mu_cvr_vec = tf.compat.v1.keras.layers.Dropout(
                        1 - self.keep_prob[1])(mu_cvr_vec)
                    mu_cvr_vec = tf.compat.v1.layers.dense(mu_cvr_vec, 256/2, activation=tf.nn.relu,
                                                 name="vector_layer_second" + str(i))
                    mu_cvr_vec = tf.compat.v1.keras.layers.Dropout(
                        1 - self.keep_prob[2])(mu_cvr_vec)

                    mu_cvr_vec = tf.compat.v1.layers.dense(mu_cvr_vec, 128/2, activation=tf.nn.relu,
                                                 name="vector_layer_third" + str(i))
                    cur = tf.compat.v1.layers.dense(mu_cvr_vec, 1, activation=None,

                                          name="output_layer_" + str(i))

                    cur_sigma = tf.compat.v1.layers.dense(mu_cvr_vec, 1, activation=tf.nn.relu,
                                                name="sigma_output_layer_" + str(i))
                    cvr_logits = tf.compat.v1.layers.dense(mu_cvr_vec, 1, activation=tf.nn.sigmoid,
                                                 name="cvr_output_layer_" + str(i))
                    cur_sigma = tf.compat.v1.math.maximum(tf.keras.backend.softplus(cur_sigma), tf.math.sqrt(tf.keras.backend.epsilon()))
            ...
            with tf.compat.v1.variable_scope('selector'):
                selector = tf.compat.v1.layers.dense(H, 1024,
                                           activation=tf.nn.relu, name="vector_layer")
                selector = tf.compat.v1.layers.dense(selector, 512,
                                           activation=tf.nn.relu, name="vector_layer2")
                selector = tf.compat.v1.layers.dense(selector, 256,
                                           activation=tf.nn.relu, name="vector_layer3")
                selector = tf.compat.v1.layers.dense(selector, num_distribution, activation=None)
            ...
            for i in range(num_distribution):
                ...
                if i == 0:
                    ...
                    final_fee_res = tf.concat([final_fee_res, tf.reshape(res_dict['res_' + str(i)], shape=[-1, 1])],
                                              axis=1)
            gumbel_select,_ = gumbel_softmax_sample(tf.math.log(tf.nn.softmax(selector,axis = 1)),0.1)
            ...
            for i in range(num_distribution):
                ...
                fee_reg = -positive * (
                        -tf.math.log(tf.sqrt(2 * 3.1415926) * cur_fee_sig_logits * save_label) - (
                        cur_fee_u_logits - tf.math.log(save_label)) * (
                                cur_fee_u_logits - tf.math.log(save_label)) / (
                                    2 * cur_fee_sig_logits * cur_fee_sig_logits) + tf.math.log(
                    tf.maximum(tf.reshape(ori_pred[:, i], shape=[-1]), 0.000001)))

                fee_neg = -(1 - positive) * (tf.math.log(tf.maximum(1 - tf.reshape(ori_pred[:, i], shape=[-1]), 0.0000000001)))
            ...
            for i in range(num_distribution):
                d_loss +=   tf.reshape(gumbel_label[:, i], shape=[-1]) *( tf.math.log(tf.maximum(tf.reshape(gumbel_label[:, i], shape=[-1]),0.0000000001 )) - tf.math.log(
            ...
            for i in range(num_distribution):
                ...
                cur_loss = tf.reduce_mean( -cur_min * ((1 -  cur_dis) ** 2) * tf.math.log(cur_dis)) #selector_loss #tf.reshape(selector_loss[:,i])
            ...
            # init
            self.saver = tf.compat.v1.train.Saver(var_list=tf.compat.v1.global_variables())
    ...
    def _initialize_weights(self):
        ...
        '''
        all_weights = dict()
        l2_reg = tf.keras.regularizers.L2(self.lamda)
        # attention
        all_weights['attention_w1'] = tf.compat.v1.get_variable(
            initializer=tf.random.normal(
                shape=[32, 32],
                mean=0.0,
                stddev=0.01),
            regularizer=l2_reg, name='attention_w1')  # k * k
        all_weights['attention_w2'] = tf.compat.v1.get_variable(
            initializer=tf.random.normal(
                shape=[32, 32],
                mean=0.0,
                stddev=0.01),
            regularizer=l2_reg, name='attention_w2')  # k * k
        all_weights['attention_w3'] = tf.compat.v1.get_variable(
            initializer=tf.random.normal(
                shape=[32, 32],
                mean=0.0,
                stddev=0.01),
        ...
        # embedding
        for column in all_columns:
            all_weights['feature_embeddings_{}'.format(column)] = tf.compat.v1.get_variable(
                initializer=tf.random.normal(
                    shape=[
